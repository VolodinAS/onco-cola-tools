...
- **`halt, quiet`**: Функции-прерыватели для принудительного прерывания исполнения (аналог exit)
- **`ProgressMessage`**: Класс для вывода прогресса и времени, затраченного на исполнение
- **`DebugMixin`**: Класс-миксин для хранения переменной _debug: bool - использовать, чтобы решить - выводить какой-то принт или нет.
